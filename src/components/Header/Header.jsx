...
function Header({ loggedIn, authorized }) {
    ...
    <header className={`header ${authorized ? 'header_theme_white' : ''}`}>
        <div className='header__container'>
            ...
            <nav className={`header__nav ${loggedIn || authorized ? 'header__nav_visible' : ''}`}>
                <ul className='header__list'>
                    ...
                    <li className={`header__acc ${loggedIn || authorized ? 'header__acc_visible' : ''}`}>
                        <NavLink to='/profile' className='header__link header__profile header__pic'>Аккаунт</NavLink>
                    </li>
                </ul>
            </nav>

            <button className={`header__menu ${loggedIn || authorized ? 'header__menu_visible' : ''}`} onClick={openBurger}>
                <span className='header__span'/>
            </button>
            <div className={`header__but ${loggedIn || authorized ? 'header__but_hidden' : ''}`}>
                <Link to='/signup'>
